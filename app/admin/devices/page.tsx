...
export default function DeviceManagementPage() {
    ...
    const { data: sessionData, isLoading } = useQuery({
        ...
        queryFn: () => api.get('/sessions').then(res => res.data.sessions)
    });

    const logoutMutation = useMutation({
        mutationFn: (tokenId: number) => api.delete(`/sessions/${tokenId}`),
        onSuccess: () => {
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <div className="bg-primary/5 px-4 py-2 rounded-2xl border border-primary/10">
                        ...
                        <p className="text-xl font-black italic">{sessions.length}<span className="text-on-surface/20">/4</span></p>
                    </div>
                    <Button 
            ...
            <div className="bg-slate-900 rounded-[2rem] p-8 text-white flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative">
                ...
                <div className="flex items-center gap-6 relative z-10">
                    ...
                    <div>
                        ...
                        <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest">To maintain network integrity, a maximum of 4 devices are allowed per node.</p>
                    </div>
                </div>
